...
func (S *Service) createChannel(connectId int, conn *amqp.Connection) channel {
	...
	//开启confirm模式 https://studygolang.com/articles/29434
	ch.Confirm(false)
...
func (S *Service) publish(channelId int, ch *amqp.Channel, exchangeName string,
	...
	if err != nil {
		if delayTime > 0 {
			if strings.Index(err.Error(), "channel/connection is not open") >= 0 {
				err = S.rePublish(channelId, exchangeName, err, routeKey, data, delayTime)
			}
		} else {
			if strings.Index(err.Error(), "channel/connection is not open") >= 0 {
				err = S.rePublish(channelId, exchangeName, err, routeKey, data, delayTime)
...
func (S *Service) rePublish(channelId int, exchangeName string,
	errmsg error, routeKey string, data string, delayTime int) (err error) {
